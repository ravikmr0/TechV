...
export function Hero() {
  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12">
        <div className="relative z-10 grid items-center gap-4 sm:gap-6 md:gap-10 lg:gap-12 xl:gap-16 py-8 sm:py-12 md:py-20 lg:grid-cols-2 lg:py-24 xl:py-28 min-h-[100svh]">
          <div className="w-full max-w-2xl mx-auto lg:mx-0 text-center lg:text-left space-y-3 sm:space-y-4 md:space-y-6">
            ...
            <motion.div
              className="overflow-hidden rounded-full border border-white/10 bg-white/5 backdrop-blur-sm px-2.5 py-2 sm:px-3 sm:py-2.5 md:px-4 md:py-3"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.7, delay: 0.3 }}
            >
              <motion.div
                className="flex items-center gap-2 sm:gap-2.5 md:gap-3 whitespace-nowrap"
                animate={{ x: ["0%", "-50%"] }}
                transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
              >
                {[0, 1].map((row) => (
                  <div key={row} className="flex items-center gap-2 sm:gap-2.5 md:gap-3">
                    {tags.map((tag, index) => (
                      <motion.span
                        key={`${row}-${index}`}
                        className="rounded-full bg-gradient-to-r from-cyan-500/80 to-blue-500/80 px-2.5 py-1 sm:px-3 sm:py-1 md:px-4 md:py-1.5 text-xs sm:text-sm font-medium text-white shadow-lg"
            ...
            <motion.div
              className="h-1 w-20 sm:w-24 md:w-32 rounded-full bg-gradient-to-r from-cyan-400 via-blue-400 to-violet-400 mx-auto lg:mx-0"
            ...
            >
              <motion.span
                className="inline-flex items-center gap-1 sm:gap-1.5 rounded-full border border-emerald-400/30 bg-emerald-400/10 backdrop-blur-sm px-2 py-1 sm:px-2.5 sm:py-1.5 md:px-3.5 text-xs sm:text-sm text-emerald-200 shadow-lg"
              ...
              <motion.span
                className="inline-flex items-center gap-1 sm:gap-1.5 md:gap-2 rounded-full border border-white/20 bg-white/10 backdrop-blur-sm px-2 py-1 sm:px-2.5 sm:py-1.5 md:px-3.5 text-xs sm:text-sm"
                whileHover={{ scale: 1.05 }}
              >
                <span className="text-white/70">Success</span>
                <span className="rounded-full bg-white/25 px-1.5 py-0.5 sm:px-2 font-semibold text-white text-xs sm:text-sm">96%</span>
              </motion.span>
              <motion.span
                className="rounded-full border border-white/20 bg-white/10 backdrop-blur-sm px-2 py-1 sm:px-2.5 sm:py-1.5 md:px-3.5 text-xs sm:text-sm text-white/70"
                whileHover={{ scale: 1.05 }}
              >
          ...
          <div className="w-full max-w-md sm:max-w-lg mx-auto lg:max-w-none order-first lg:order-last -mt-4 sm:mt-0">
            ...
            >
              <motion.div
                className="absolute inset-0 rounded-xl sm:rounded-2xl md:rounded-3xl border border-white/10 bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-sm shadow-2xl overflow-hidden"
                whileHover={{ scale: 1.02 }}
                transition={{ type: "spring", stiffness: 300 }}
              >
                <div className="pointer-events-none absolute inset-0 rounded-xl sm:rounded-2xl md:rounded-3xl [mask-image:radial-gradient(ellipse_at_center,black_40%,transparent_80%)]">
                  <div
              ...
              <motion.div
                className="absolute -inset-2 sm:-inset-3 md:-inset-4 rounded-xl sm:rounded-2xl md:rounded-3xl bg-gradient-to-r from-cyan-500/20 via-blue-500/20 to-violet-500/20 blur-xl sm:blur-2xl md:blur-3xl -z-10"
                animate={{
                  opacity: [0.3, 0.6, 0.3],
                  scale: [1, 1.05, 1],
                }}
